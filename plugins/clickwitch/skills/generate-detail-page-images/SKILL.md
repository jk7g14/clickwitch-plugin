...
- `clickwitch_get_started` is a quick account/workspace summary and may show only a small slice. Use `clickwitch_list_assets` for selection work: page through `workspaces`, `products`, `brands`, `concepts`, `advertising_models` and `categories` with the real `workspace_id`, `query`, `limit` and `cursor` values. Apply `category_top` to category/concept selection and `modelMode` to concept/model eligibility. An omitted item is not absent. Keep paging or inspect the existing workspace UI before creating replacements.
...
## Choose filtered inputs before creating a pack

When the user asks to rebuild, compare or create packs from existing choices, reuse those existing assets first. Do not recreate the same brand, product, concept or model just because it is not in the first summary page.

1. Resolve the intended workspace with `clickwitch_list_assets(kind: "workspaces")`, using `query` when the user named a brand, campaign or saved workspace. Use the exact `workspace_id` from the selected workspace for all following lists.
2. List products and brands in that workspace. Select only ready products and ready brands unless the user explicitly asked to repair missing assets first. For multi-product sets, preserve every selected product id and pass a matching `product_roles` entry for each id; keep the single main product first.
3. List `categories` and choose the nearest ready `category_top` for the product domain. Then list `concepts` with that `category_top` and the desired `modelMode`. Treat the chosen concept as the ClickWitch theme. Do not invent theme or category ids.
4. If the pack requires a human model, list `advertising_models` with the chosen `workspace_id`, a practical `query` when needed, and `modelMode: "require_model"`. Select only `readiness.state: "ready"` candidates unless the task is specifically to repair or build a model asset. If the pack is product-only, pass `modelMode: "product_only"` and do not pick an advertising model.
5. Search templates only as a reference signal before pack creation; `template_id` must come from the current server's supported create-pack contract. A pretty template hit is not proof that every section will receive its raw layout image.
6. For language matrices such as Korean, Japanese and English, create one run per requested `language` while holding workspace, products, brand, category, concept/theme, model mode, advertising-model choice and product roles fixed unless the user asks for different local adaptation.
7. For baseline or A/B comparisons, keep a manifest that labels each run/output by input set, language and host rendering surface. Do not widen the comparison matrix beyond the user's requested variants.

## Read readiness and saved state

1. Call `clickwitch_get_started` in the intended workspace. For new/rebuilt packs, refine the selected assets with `clickwitch_list_assets` before calling `clickwitch_create_pack`. For a known run, read `clickwitch_get_run` with `response_format: "concise"`. Only create a new pack when the user's request needs one and inputs/credit approval are ready.
2. Wait for actual `ready` status with bounded retries appropriate to the host/runtime. If status stays pending but other independent work remains, continue that work before reporting. If there is no useful independent work left, report the current state and resumable URL. Read the ready run with `response_format: "detailed"` for exact section IDs and order. If truncated, request its omitted section IDs with `section_ids`; never infer IDs from titles/templates.
...
- **Prepared kit:** when `clickwitch_prepare_section_image` is available, call it with the real run/section IDs. Use **structuredContent.prompt** when available; otherwise extract only the fenced **실행 프롬프트** from the first text block, not its metadata, host instructions or footer. Pair that exact prompt with **all ordered MCP image blocks immediately after the text block**, only for this section. For HOOK/HERO sections, inspect the returned reference roles before rendering. If product/model references are present but the visual layout/template reference is absent, record that the section is text-only for template guidance and do not claim full HERO-template fidelity unless a supported layout reference is restored or the user accepts the limitation.
